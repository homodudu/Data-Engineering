...
class TestXetraETLMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        conf_dict_src = {
            'src_first_extract_date': '2022-12-01',
        ...
        # Create source files on mocked s3
        columns_src = ['ISIN', 'Mnemonic', 'Date', 'Time', 'StartPrice',
        'EndPrice', 'MinPrice', 'MaxPrice', 'TradedVolume']
        data = [['AT0000A0E9W5', 'SANT', '2022-12-15', '12:00', 20.19, 18.45, 18.20, 20.33, 877],
                ['AT0000A0E9W5', 'SANT', '2022-12-16', '15:00', 18.27, 21.19, 18.27, 21.34, 987],
                ['AT0000A0E9W5', 'SANT', '2022-12-17', '13:00', 20.21, 18.27, 18.21, 20.42, 633],
                ['AT0000A0E9W5', 'SANT', '2022-12-17', '14:00', 18.27, 21.19, 18.27, 21.34, 455],
                ['AT0000A0E9W5', 'SANT', '2022-12-18', '07:00', 20.58, 19.27, 18.89, 20.58, 9066],
                ['AT0000A0E9W5', 'SANT', '2022-12-18', '08:00', 19.27, 21.14, 19.27, 21.14, 1220],
                ['AT0000A0E9W5', 'SANT', '2022-12-19', '07:00', 23.58, 23.58, 23.58, 23.58, 1035],
                ['AT0000A0E9W5', 'SANT', '2022-12-19', '08:00', 23.58, 24.22, 23.31, 24.34, 1028],
                ['AT0000A0E9W5', 'SANT', '2022-12-19', '09:00', 24.22, 22.21, 22.21, 25.01, 1523]]
        self.df_src = pd.DataFrame(data, columns=columns_src)
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[0:0],
        '2022-12-15/2022-12-15_BINS_XETR12.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[1:1],
        '2022-12-16/2022-12-16_BINS_XETR15.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[2:2],
        '2022-12-17/2022-12-17_BINS_XETR13.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[3:3],
        '2022-12-17/2022-12-17_BINS_XETR14.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[4:4],
        '2022-12-18/2022-12-18_BINS_XETR07.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[5:5],
        '2022-12-18/2022-12-18_BINS_XETR08.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[6:6],
        '2022-12-19/2022-12-19_BINS_XETR07.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[7:7],
        '2022-12-19/2022-12-19_BINS_XETR08.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[8:8],
        '2022-12-19/2022-12-19_BINS_XETR09.csv','csv')
        ...
        data_report = [['AT0000A0E9W5', '2022-12-17', 20.21, 18.27, 18.21, 21.34, 1088, 10.62],
                       ['AT0000A0E9W5', '2022-12-18', 20.58, 19.27, 18.89, 21.14, 10286, 1.83],
                       ['AT0000A0E9W5', '2022-12-19', 23.58, 24.22, 22.21, 25.01, 3586, 14.58]]
    ...
    def test_extract_files(self):
        ...
        # Test init
        extract_date = '2022-12-17'
        extract_date_list = ['2022-12-16', '2022-12-17', '2022-12-18', '2022-12-19', '2022-12-20']
        # Method execution
        with patch.object(MetaProcess, "return_date_list",
        return_value=[extract_date, extract_date_list]):
            xetra_etl = XetraETL(self.s3_bucket_src, self.s3_bucket_trg,
    ...
    def test_transform_report1_empty_df(self):
        """
        Tests the transform_report1 method with empty DataFrame as input argument
        """
        # Expected results
        log_exp = 'The dataframe is empty. No transformations will be applied.'
        # Test init
        extract_date = '2022-12-17'
        extract_date_list = ['2022-12-16', '2022-12-17', '2022-12-18']
    ...
    def test_transform_report1_ok(self):
        """
        Tests the transform_report1 method DataFrame as input argument
        """
        # Expected results
        ...
        # Test init
        extract_date = '2022-12-17'
        extract_date_list = ['2022-12-16', '2022-12-17', '2022-12-18', '2022-12-19']
    ...
    def test_load(self):
        ...
        df_exp = self.df_report
        meta_exp = ['2022-12-17', '2022-12-18', '2022-12-19']
